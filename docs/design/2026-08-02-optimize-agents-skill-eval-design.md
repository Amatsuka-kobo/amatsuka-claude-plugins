...
| 事項 | 状況 |
| --- | --- |
| `agent-creator` の fp 1 件 | 「Claude Code の subagent の仕組みについて説明して」が 3/10 で誤発火する。除外文を 3 通り試した結果が最良の状態。詳細は下表 |
| ローカルプロキシ(`127.0.0.1:8317`)の上流 | 調査しない(ユーザー判断)。サブスク認証か API キーかは未確定。測定は `environment` に経路を記録することで、後から判断できる状態にする |

### `agent-creator` の除外文の実測(2026-08-02)

| 書き方 | 誤発火 |
| --- | --- |
| A: 「subagent や Claude Code の仕組み・委譲の判断基準を説明するだけの質問には使わない」(採用) | 3/10 |
| B: 「〜の仕組みを教えて」「〜について説明して」「〜とは何か」を列挙 | 5/10 |
| C: 「知識を問うだけの質問では使わない」(短く抽象的に) | 9/10 |

除外文を抽象化すると悪化する。クエリとほぼ同一の文字列を入れても悪化する。具体語で対象を名指しするのが最も効く。

## 18. 実装で判明した事実(2026-08-02)

### eval セットに指示語を入れない

trigger eval は 1 発のクエリだけを投げるので、「この」「さっきの」のような指示語は参照先を持たない。対象不明と判断され、発火が抑えられる。

| クエリ | 発火 |
| --- | --- |
| 「この SKILL.md の description を直して」 | 1/4 |
| 「chat スキルの SKILL.md の description を直して」 | 4/4 |
| 「SKILL.md の description を直して」 | 3/4 |

実運用の会話には先行文脈があるため、この差は測定にだけ現れる。`should_trigger: true` のクエリに指示語があると偽の発火漏れになる。

この規律は `skill-eval` の本文に「eval セットの書き方」として記載した。

### 測定のばらつき

同一クエリを 10 回ずつ測っても結果が動く。連続測定では先に走った方が高く出る傾向がある。
...
168 問の回帰で fp が 66/72(基準 69/72)となったが、この分散として扱った。1〜2 問の差で実装や description を疑わない規律を `skill-eval` に記載した。
