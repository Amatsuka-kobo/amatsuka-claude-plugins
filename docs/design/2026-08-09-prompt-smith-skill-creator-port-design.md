...
公式 `skill-creator` は description 改善ループ(測る → 直す → 測り直す → 最良を選ぶ)を持つが、その測定部 `run_eval.py` はこのリポジトリで使えない。理由は 2 つある。

**1. 実リポジトリを cwd にする。** `find_project_root()` は `.claude/` を持つ祖先を探して cwd にする。このリポジトリで走らせると、45 のプロジェクトスキルが同席した状態で測ることになる。得られるスコアは description の質ではなく、手元のカタログとの競争結果である。

**2. 本文を捨てた薄いファイルを測る。** 測定対象として description だけを載せたファイルを組み立てる(`run_eval.py:58-68`)。本番の SKILL.md とは別物を測っている。

この 2 つに気づかず description を書き換えると、実態と違う測定に合わせて本番を改悪する。

### 登録先については当初の診断が誤っていた

設計の初版は「測定対象を `.claude/commands/` へ登録するため、自然文の依頼から選ばれず、結果が常に『発火せず』になる」と診断していた。**2026-08-09 の対照実験でこの診断は否定された。**

同一 description・同一クエリ(8 問 × 3 回)を、条件を揃えた一時ディレクトリの `.claude/skills/` と `.claude/commands/` の両方で測った結果である。
...
差が出たのは 8 問中 2 問で、方向は逆である。ばらつきの範囲であり、登録先は判別要因ではない。

公式ドキュメントもこれを裏付ける。`.claude/` ディレクトリの解説は、`commands/deploy.md` と `skills/deploy/SKILL.md` について "**Both can be auto-invoked by Claude**" と明記している。

出典: [The .claude directory](https://code.claude.com/docs/en/claude-directory) / [Extend Claude with skills](https://code.claude.com/docs/en/skills)(2026-08-09 参照)、および上記の実測。

### 2026-08-01 の実測は交絡していた

抗体 `ab-2026-0802-001` は、登録先を `.claude/skills/` に変えると発火が 1/8 から 8/8 になったと記録している。この差は**登録先だけで生じたものではない**。

削除済みの旧測定器 `plugins/optimize-agents/scripts/run-trigger-eval.mjs` は、93 行目で `mkdtempSync(join(tmpdir(), "trigger-eval-"))` を作り、117 行目でそれを cwd にしていた。一方 `run_eval.py` は実リポジトリのルートを cwd にする。

| 実行 | 登録先 | cwd | 結果 |
| --- | --- | --- | --- |
| `run_eval.py` | `.claude/commands/` | 実リポジトリのルート(45 スキルが同席) | 1/8 |
| 旧 `run-trigger-eval.mjs` | `.claude/skills/` | 一時ディレクトリ(測定対象だけ) | 8/8 |

**2 つの変数が同時に変わっている。** この比較から登録先を原因と特定することはできない。

2026-08-09 の実験は cwd を揃えて登録先だけを変え、差が出なかった。両方を突き合わせると、**効いていたのは cwd の方**だったと読むのが自然である。1/8 は「コマンドだから発火しなかった」のではなく「45 のスキルとの競争に負けた」結果と考えられる。

これは §1 冒頭で挙げた移植の理由 1(実リポジトリを cwd にする)を裏付ける。

なお、commands と skills の統合が**いつ入ったか**は特定できていない。リリースノートに該当の記載を見つけられなかった。ただし上の説明は統合の時期に依存しないので、特定しなくても成立する。

登録先を `.claude/skills/` にする決定自体は維持する。本番の配置と一致し、`disable-model-invocation` を明示できるためである。ただし**それは移植の理由ではない**。移植の理由は上記の 2 点である。
...
| # | `run_eval.py` | 移植版 | 根拠 |
| --- | --- | --- | --- |
| 1 | `<project>/.claude/commands/<clean>.md` に登録 | `<tmp>/.claude/skills/<clean>/SKILL.md` に登録し、frontmatter の `disable-model-invocation` を `false` にする | 本番の配置と一致させ、`disable-model-invocation` を明示できるようにする。**登録先そのものは発火率に影響しない**(§1 の実測) |
...
| 公式 `skill-creator` プラグインの `run_eval.py` / `run_loop.py` は使わない | 実リポジトリを cwd にするため手元のスキルとの競争を測ることになり、かつ本文を捨てた薄いファイルを測る |
...
| §書く内容(10-21 行) | 変更しない |
| §発火率を上げるための施策(23-28 行) | 節ごと無くなる。skill 専用の 2 行(27-28)は §8.2 へ、Agents 専用の 1 行(26)は `agent-creator` の本文へ移す。移動により「Agents 定義には適用しない」(25 行)が指す対象が消えるため、この但し書きも削る |
...
この形にすると、どちらの読み手も自分に適用されない記述を読まずに済み、共通の基準は 1 箇所にとどまる。両者が将来ずれる余地も残らない。

| 文書 | 持つもの |
| --- | --- |
| `references/description-guide.md` | 共通の基準(§書く内容、配布時の固有名の扱い) |
| `skills/skill-creator/SKILL.md` | skill・コマンド定義に固有の規律(長さの上限、言い切る書き方) |
| `skills/agent-creator/SKILL.md` | Agents 定義に固有の規律(「積極的に使用する」と書く) |
...
**この検証は 2026-08-09 に実施済みである。結果は §1 に記した。**

同一 description・同一クエリを、条件を揃えた一時ディレクトリの `.claude/commands/` と `.claude/skills/` の両方で測った。両経路とも 10/24 で、登録先は判別要因ではなかった。判定表の「どちらも同じだけ発火する」に当たったため、実装ではなく前提を疑い、§1・§4.1#1・§8.1 の記述を実測に合わせて改めた。

判定表そのものは、将来 Claude Code の版が変わったときに再実施するために残す。

| 観測 | 疑う対象 | 次に行うこと |
| --- | --- | --- |
| skills 経路だけ発火する | — | 登録先が判別要因に戻ったということ。§1 を再度改める |
| どちらも発火しない | 測定器の実装、またはタイムアウト | `--timeout` を伸ばして再測。stream-json の生ログを読み、`Skill` の `content_block` が来ているかを直接見る |
| どちらも同じだけ発火する | — | 2026-08-09 と同じ。現状維持 |

これは単体テストではなく、手で行う検証手順である。§10 の「`claude -p` の実起動はテストしない」と矛盾しない。

**「差が出ない = 実装不良」と決めつけない。** この判定表を先に用意していたため、同数という結果を実装バグと誤診せずに済んだ。
...
| ~~`.claude/commands/` が将来 自動ロードされるようになる~~ | **2026-08-09 に発現済み。** §1 の登録先の診断が失効した | 移植の理由を cwd 汚染と薄いファイルの 2 点に差し替えた。実装は変えていない |
...
| 事項 | 状況 |
| --- | --- |
| 「explain the why」が移植先に残らない | **2026-08-10 に判断済み。放置する。** 公式 `skill-creator` の Writing Style は「MUST を並べず、なぜ重要かを説明せよ」と書くが、`prompt-smith` は「指示を正当化する根拠・理由は削り、指示だけを残す」と定めており、方向が逆である。本文を `prompt-smith` 準拠にする決定を優先し、この 1 点は移植先に持ち込まない |
| `<new_description>` タグが無い応答の扱い | **2026-08-10 に判断済み。移植元どおりのままとする。** タグが無い応答は全文を新 description として採用する。2026-08-09 の実地確認で 1 度、`claude -p` が無関係なテキストをタグ無しで返し、それが採用される事象を観測した。ただしループの最良選択が働き、スコアの下がった反復は棄却された。翌日の健全性確認では `claude -p` は正常に応答しており、一過性と判断する |
