...
5. スクラッチファイルの frontmatter(`urgency` 等)と本文を読む。本文が空、またはテンプレートの「(ここにコメント本文)」のまま変更されていない場合は**何もせず**スクラッチファイルを削除する(送信しない)
...
`watchPitcrew` のコールバックで `.pitcrew/` の変更を検知するたびに `listState`/`readItems` を再取得し、**選択中アイテムの `id` を追跡**して選択位置を維持する(該当 id が一覧から消えていれば、直前の位置にクランプする)。キー入力(`readline.emitKeypressEvents` + raw mode)は別イベントソースとして共存させ、どちらのイベントが来ても同じ描画関数(`render.ts`)を呼ぶ。
