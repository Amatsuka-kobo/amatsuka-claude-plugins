...
├── run.lock                 # run.json 更新の排他制御用ロックファイル(§6。存在は一時的)
...
- PreToolUse hook は `comments/` に urgent があるときだけパス照合を行い、
  マッチしたら **先に `processed/` への rename でコメントを獲得(クレーム)してから**
  hook 出力(`hookSpecificOutput.additionalContext`)として本文を注入する。
  rename の原子性により、複数のサブエージェントが同じパスを触っていても受け取るのは
  rename に成功した 1 エージェントのみ(早い者勝ち。§6「並行動作と競合の解決」)。
  同時に複数の urgent がマッチした場合は作成順にすべて連結して注入する。
  `permissionDecision` は返さない(権限フローは通常どおりで、ツール実行を妨げない)
- Stop hook は未回収コメントを **先に `processed/` への rename で獲得してから**
  まとめて差し戻し(`decision: block` + 理由にコメント本文)、メインに反映を促す。
  次の Stop 発火時には未回収コメントが残っていないため再差し戻しは起きない
  (重複防止はこのファイル移動が担う)
- パス照合はコメント frontmatter の `paths` とツール入力の単純比較。
  完全一致、または**コメント側のパスがツール入力パスの祖先ディレクトリ**である場合にマッチする。
  決定的で LLM を使わない
...
- Stop hook の差し戻しは未回収コメントがあるときのみ。加えて Claude Code が
  hook 入力で渡す `stop_hook_active: true`(直前の Stop 差し戻しから継続中のターン)の場合は
  差し戻さない。この二重ガードで無限ループを防ぐ

### 取り残し防止(Stage 2 で確定)

- パスにマッチしないまま残った urgent コメントは、ターン境界(Stop)で normal と
  一緒に回収する(urgent の「即時」はベストエフォートであり、ターン境界が最終防衛線)。
  §7 の「即時のみ」モードの Stop 回収と同じ理屈をハイブリッド(既定)にも適用する
- PreToolUse の `additionalContext` には上限(10,000 文字)があるため、連結後に
  超過する場合は末尾を切り詰め、全文への参照(`processed/` のファイルパス)を付記する

### 並行動作と競合の解決(Stage 2 で確定)

Stage 1 の既知の制限(§10.5)だった並行競合は、操作の性質ごとに 2 つの機構で解決する:

1. **注入側 — rename をクレーム操作とする(ロック不要)**:
   コメントの回収は「`comments/` → `comments/processed/` への rename」そのものを
   所有権の獲得(クレーム)とする。同一ファイルへの rename は POSIX 上原子的で、
   並行する複数の hook が同じコメントを狙っても成功するのは 1 プロセスだけ
   (敗者は ENOENT を受け、そのコメントをスキップする)。注入は
   「rename に成功したコメントだけ」を対象にするため、重複注入は構造的に起きない
   - トレードオフ: rename 成功後・注入出力前にプロセスが死ぬと、そのコメントは
     注入されないまま `processed/` に移る(at-most-once 配送)。重複注入
     (同じ指示を複数エージェントが実行して作業が衝突する)よりも取りこぼし
     (人間がビューアで `processed/` を見て再投稿できる)の方が復旧が容易なため、
     at-most-once を選ぶ
2. **捕捉側 — run.json 更新はアドバイザリロックで直列化**:
   `loadRun → レビュー項目書き出し → saveRun` の read-modify-write 区間を
   `.pitcrew/run.lock` の排他作成(`O_CREAT|O_EXCL`)で直列化する
   - ロックファイルには `{ pid, acquiredAt }` を書き、`finally` で必ず削除する
   - 取得失敗時は短い間隔(50ms 程度)でリトライし、待機は合計 3 秒まで
     (hook timeout 15 秒に対して十分な余裕を持たせる)
   - **stale ロックの回収**: ロックファイルの mtime が 10 秒より古い場合は
     保持プロセスが異常終了した(hook timeout で kill された等)とみなして削除し、
     取得を再試行する。ロックで包むのは run.json の read-modify-write 区間のみ
     (git スナップショット等の重い処理はロック外)で正常保持はミリ秒オーダーのため、
     10 秒は正常動作と異常終了を分ける閾値として十分に保守的である
   - **フェイルオープンの維持**: 待機予算を使い切っても取得できない場合は、
     エラーログに記録した上で**ロックなしで処理を続行**する(Stage 1 の既知制限と
     同じ挙動への劣化であり、セッションを止めるよりも重複 diff を許容する)
- ビューア層の読み取り保護は従来どおり「一時ファイル → rename」の原子的書き込みが担い、
  読み取り側のロックは導入しない(§9)
...
- 書き込みは「一時ファイル→rename」で行い、ビューアが書きかけのファイルを読まないようにする
  (同一ディレクトリ内 rename の原子性を利用)。読み取り側のロックは導入しない。
  例外として `run.json` の read-modify-write のみ、hook の並行起動に備えて
  アドバイザリロックで直列化する(§6「並行動作と競合の解決」。ロック取得に
  失敗してもフェイルオープンで続行する)
...
## 10.5 既知の制限(Stage 1 時点 → Stage 2 で解決)

- **捕捉 hook の並行起動競合**: 複数サブエージェントがほぼ同時に終了すると SubagentStop hook が
  並行起動し、`run.json` の read-modify-write が競合し得る(レビュー ID の衝突、
  `lastCaptureCommit` の lost update による diff の重複/欠落)。
  **→ Stage 2 で解決**: `run.json` 更新をアドバイザリロック(`run.lock` の O_EXCL)で直列化し、
  注入側はロックを使わず rename 自体をクレーム操作とする。§6「並行動作と競合の解決」参照。
  ロック取得に失敗した場合のみ Stage 1 と同じ挙動(重複 diff の許容)に劣化する。
