...
5. **報告** — chat-reader の回答を出典付きで提示。`unindexed` があれば「索引に載っていない記録が N 件あります(依頼があれば今追記します)」と一言添える。ユーザーが補完を依頼した場合のみ: chat-reader に unindexed ファイルの要旨(1 行)を返させ、**スキル(メインセッション)が INDEX.md に行を追記**する(自動では行わない。chat-reader は読解専用で Write を持たない)
...
2. **対象記録の特定** — `node "${CLAUDE_PLUGIN_ROOT}/scripts/find-chat-records.mjs" --latest [N] --user <git config user.name>`
   ...
   - ユーザーがトピックを指定した場合は `--latest` ではなくキーワード検索で特定する。キーワード検索で複数ヒットした場合は、`--latest` で複数件返った場合と同じ扱い(トピック一覧から選んでもらう)に合流する
3. **読解の委譲** — `chat-reader` に対象パスを渡し、次のスロットを構造化して返させる: 前回の要旨 / 持ち越し事項 / 直近の決定と理由 / 出典
   - **持ち越し事項の定義**: 記録ファイル**全体の末尾**にある「注意事項と次の作業」セクションの内容を原文ベースで返す(chat スキルのファイル構成では末尾セクションはファイルに 1 箇所)。セッションごとの本文から chat-reader が独自に「未完了そうなもの」を推測して集めることはしない — 末尾セクションが空・欠落ならその旨を返す
...
5. **分析** — 提案一覧を作る:
   ...
   - **生死**: stale な Issue ごとに「クローズ提案(理由付き)」または「確認コメント提案」を選んで提案。選択基準は決定的ルール: **クローズ提案にできるのは「自分(現在の GitHub ログインユーザー)が作者で、アサインなし・他者コメントなし」の Issue のみ**。それ以外の stale Issue はすべて確認コメント提案とする(LLM の裁量で判断させない)
...
- 「自分/他者」の判定は **GitHub のログインユーザー**(`gh api user` の `login`、または MCP Tool の相当情報)で行う。git のユーザー名(`git config user.name`)は chat 記録用であり、GitHub 上の author/assignees との照合には使わない。アサインが複数人の場合、1 人でも他者がいればクローズ提案禁止
...
- **--latest モード**: パスの日付構造から新しい順に N 件(既定 3)のパス+タイトル(先頭 `# ` 行)を返す。日付ソートは決定的にできることなので LLM でなくコードで行う
  - ソートキーは `YYYY/MMDD` の文字列を数値化したもの(年またぎも正しく順序付く)。パス上の日付は記録時のローカル日付であり、タイムゾーン変換はしない(記録と検索が同じマシンで行われる前提で十分)
  - 同日内の複数ファイルはファイルの mtime 降順で並べる(同日はほぼ同じ文脈なので厳密性は要求しない)
...
- 手順に追加: 記録ファイルの作成/追記後、INDEX.md の対応行を追加または更新(なければヘッダー `# Chat Records Index` 付きで新規作成)。chat-recorder は Stop フックの差し戻しにより 1 ストップ 1 回しか起動されないため、同時書き込みの排他制御は設けない
