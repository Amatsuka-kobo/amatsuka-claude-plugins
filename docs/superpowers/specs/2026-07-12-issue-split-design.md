...
2. **操作手段の決定** — GitHub 操作系 MCP Tool(Issue 作成 + Sub-issue リンクの両方ができるもの)があればそれを優先。なければ `gh`(未導入/未認証なら案内して STOP)。Issue 作成だけできる MCP Tool の場合は、作成は MCP・リンクは `link-sub-issue.mjs` の併用とする。
