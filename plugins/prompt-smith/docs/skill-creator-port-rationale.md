...
2026-09-17 の隔離まで、スコアは description 単独の発火率ではなく、測定環境に常駐するスキル群との競争に勝った率である。`environment` が記録する認証経路とモデルに加え、有効なプラグインやユーザースキルの変化も比較時に考慮する必要がある。

2026-09-17 の隔離後、測定は隔離した一時環境で走り、同席するのは CLI 組み込みのスキルだけである。したがってスコアは、CLI 組み込みのスキルとの競争に勝った率である。比較時には、CLI の版、CLI 組み込みスキル、モデルの解決先の変化を考慮する。ユーザースキルとプラグインのスキルは、もはや競争相手ではない。
