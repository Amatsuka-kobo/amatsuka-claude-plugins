...
- 例は、性質が同じものだけを 1 つに絞る。性質の違う例は削らない。
...
- 残す基準に当たる内容を保てる範囲で、最も短い文で書く。
- 禁止ではなく望ましい動きや振る舞いを書く。
...
- 判断を示す例は、性質の違うものを 2 つ以上置く。
- 命令形(「〜すること」)ではなく言い切り(「〜する」「〜しない」)で書く。
...
- 新規に書くときは、素案を書き切ってから別のパスで基準を当てて削る。
