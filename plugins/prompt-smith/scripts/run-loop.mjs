...
import { mkdir as mkdir3, mkdtemp as mkdtemp2, readFile as readFile3, writeFile as writeFile4 } from "node:fs/promises";
...
function aggregateRuns(results) {
  return results.reduce(
    ([correct, total], result) => [
      correct + (result.should_trigger ? result.triggers : result.runs - result.triggers),
      total + result.runs
    ],
    [0, 0]
  );
...
function queryHeaders(queries, test = false) {
...
function resultCells(queries, results, test = false) {
...
function reportRow(record, trainQueries, testQueries, bestIteration) {
...
function generateHtml(output, autoRefresh, skillName) {
  ...
  const bestIteration = history.length ? testQueries.length ? history.reduce(
    (best, record) => (record.test_passed ?? 0) > (best.test_passed ?? 0) ? record : best
  ).iteration : history.reduce(
    (best, record) => record.train_passed > best.train_passed ? record : best
  ).iteration : void 0;
  const titlePrefix = skillName ? `${escapeHtml(skillName)} \u2014 ` : "";
  const score2 = autoRefresh ? "in progress" : output.best_score;
...
<body>
    ...
    <div class="summary">
        ...
        <p><strong>Best Score:</strong> ${score2}${autoRefresh ? "" : ` (${scoreSet})`}</p>
...
${queryHeaders(trainQueries).join("")}${queryHeaders(testQueries, true).join("")}            </tr>
        ...
        <tbody>
${history.map((record) => reportRow(record, trainQueries, testQueries, bestIteration)).join("")}        </tbody>
    </table>
    </div>
</body>
</html>
...
async function main2() {
  ...
  if (values.help) {
    process.stdout.write(renderHelp(helpSpec2));
    return;
  }
...
function openInBrowser(reportPath) {
  ...
  try {
    const browser = spawn3(command, args, { detached: true, stdio: "ignore" });
    browser.once("error", () => {
    });
    browser.unref();
  } catch {
...
async function main3() {
  ...
  if (values.help) {
    process.stdout.write(renderHelp(helpSpec3));
    return;
  }
  if (!values["eval-set"]) throw new Error("--eval-set is required");
  if (!values["skill-path"]) throw new Error("--skill-path is required");
  ...
  const reportPath = reportOption === "none" ? void 0 : autoReport ? join4(
    await mkdtemp2(join4(tmpdir2(), "prompt-smith-report-")),
    "report.html"
  ) : reportOption;
  let reportAnnounced = false;
  let browserOpened = false;
  const writeReport = (output, autoRefresh) => {
...
`);
  ...
  const result = await runLoop({
    ...
    onIteration: reportPath ? (partial) => writeReport(partial, true) : void 0
  });
