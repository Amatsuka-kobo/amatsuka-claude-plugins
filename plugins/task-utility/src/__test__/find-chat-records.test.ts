...
function fixture(files: Record<string, string>, index?: string): string {
  ...
  if (index !== undefined)
    fs.writeFileSync(path.join(dir, "docs", "chat", "INDEX.md"), index)
  return dir
}
...
test("--latest: 同日内は mtime 降順", () => {
  ...
  fs.utimesSync(
    path.join(dir, "docs/chat/2026/0101/alice/first.md"),
    atime,
    new Date("2026-01-01T10:00:00Z")
  )
  fs.utimesSync(
    path.join(dir, "docs/chat/2026/0101/alice/second.md"),
    atime,
...
test("INDEX.md が無ければ grep モード: マッチ行と前後文脈・タイトルを返す", () => {
  ...
  expect(out.hits[0].matches[0]).toMatch(
    /前の行\nストリーミング方式を採用\n次の行/
  )
})
