...
var issuesRes = gh("api", "--paginate", "--slurp", "repos/{owner}/{repo}/issues?state=open&per_page=100");
if (!issuesRes.ok) fail("issues", issuesRes.error);
var rawIssues = parsePaginated(issuesRes.stdout, "issues");
var labelsRes = gh("api", "--paginate", "--slurp", "repos/{owner}/{repo}/labels?per_page=100");
...
var labels = rawLabels.map((l) => ({ name: l.name, description: l.description ?? "" }));
