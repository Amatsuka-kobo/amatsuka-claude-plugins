---
name: issue-craft
description: ユーザーが GitHub Issue の起票・作成・下書きを依頼したときに必ず使用するスキル。ユーザーとのブレインストーミングで Issue の内容を練り上げ、プロジェクトのリモートリポジトリ(GitHub)に起票する。複数 Issue の一括起票にも対応。既存 Issue をタスク分解する場合は issue-split を使う。明示的な依頼があったときのみ使い、自律的には発動しない。
---
