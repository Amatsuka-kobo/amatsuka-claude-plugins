...
- **発火条件**: 実質的なユーザー発言が 3 回以上あり、かつセッション中に `docs/chat/` への Write/Edit がないとき。差し戻しは 1 ストップにつき 1 回まで(無限ループ防止)
- **制限**: セッション中に一度記録すると、それ以降の会話の追記は自動では促されない(モデルの裁量またはユーザーの依頼で追記する)。フック設定の変更はセッション再起動後に反映される
