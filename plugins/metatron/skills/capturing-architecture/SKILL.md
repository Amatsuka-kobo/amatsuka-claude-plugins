...
description: アーキテクチャ文書(ARCHITECTURE)がまだ無いプロジェクトで初版を作るとき、ドメインマップだけの最小構成に節を足して通常の構成へ育てるときに必ず使用する。「アーキ文書を作って」「初期化したい」「全体像の文書が無い」「この構成をドキュメントに起こして」のような依頼が該当する。規約・保護パス・テスト方針の rules 3 ファイルと失敗の台帳も併せて作る。既にある文書と実装の乖離を直す依頼は別のスキルが担当する。
---
...
- 生成物のファイルを直読させず、提示はメッセージ本文で行う。
- 修正の指示を受けたらその単位を直して再提示し、確定を得てから次の単位へ進む。
- 確定していない単位が残っているときは、その単位のウォークスルーへ戻る。
- ドメインマップは解析候補として提示し、最終決定を必ずユーザーに確認する。
...
- `body` は見出し行を含まない本文だけにする。
- `heading` に `ADR 一覧` を指定しない。
- `heading` に `テスト方針` / `保護パス` / `規約` を指定しない。指定すると `unknown_heading` で返る。
