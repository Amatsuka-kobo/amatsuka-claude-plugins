...
全体アーキテクチャ図は任意とする。図がないと全体像を誤解するときだけ Mermaid で置く。
載せるのは全体像の把握に要る要素だけとし、個々のクラス・関数・テーブルは載せない。図の構文は検証されない。
-->
...
| ORM / データ層 | Prisma 5.x |
| パッケージマネージャ | pnpm 9.x |
| 主要ライブラリ | React 18 / Zod / TanStack Query |
...
### ADR-002: RLS のテナントコンテキストをトランザクション単位で設定する
...
ADR-001 の行レベル分離では、各リクエストの認証済み `tenantId` を PostgreSQL の RLS が参照できる形でデータベースへ渡す必要がある。接続プールを使うため、設定値の残留によるテナント越境も防がなければならない。

#### 検討した選択肢

1. トランザクション開始時にセッション変数を設定する: 接続を再利用しても設定範囲をトランザクションに閉じ込められる / すべてのトランザクションで設定処理が必要になる
2. テナントごとにデータベース接続ロールを分ける: データベース側で分離を強制できる / テナント数に応じてロールと接続管理が増える
3. すべてのクエリに `tenantId` 条件を付ける: 既存の ORM の仕組みに乗せやすい / 条件の付け忘れを RLS と二重に管理することになり、漏れを防ぎきれない

#### 採用した結論

認証済みの `tenantId` をトランザクション開始時にトランザクションローカルなセッション変数へ設定し、RLS ポリシーから参照する。

#### 理由

接続プールで接続が再利用されても、トランザクションの終了時に設定が破棄されるため、前のリクエストのテナント情報が残らない。ADR-001 の RLS と組み合わせることで、アプリケーションの条件指定漏れと接続再利用による越境の両方を防げる。接続ロールをテナントごとに管理するより運用負荷が小さく、単なるクエリ条件よりも強制力がある。

#### 影響範囲

`src/server/db.ts` のトランザクション開始処理、RLS ポリシーを設定するマイグレーション、認証コンテキストから `tenantId` を受け渡すリクエスト処理。
