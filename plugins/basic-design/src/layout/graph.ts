...
function edgeLabel(text: string): ElkLabel[] | undefined {
  return text ? [{ text, width: Math.max(48, text.length * 7 + 16), height: 18 }] : undefined
}
...
function points(edge: ElkExtendedEdge) {
  ...
  return sections
    .flatMap((section, index) => [section.startPoint, ...(section.bendPoints ?? []), section.endPoint].slice(index ? 1 : 0))
    .map(({ x, y }) => ({ x: Math.round(x * 1000) / 1000, y: Math.round(y * 1000) / 1000 }))
}

function labelBox(edge: ElkExtendedEdge): Box | undefined {
  const label = edge.labels?.[0]
  if (!label || label.x === undefined || label.y === undefined || label.width === undefined || label.height === undefined) return undefined
  return { x: label.x, y: label.y, width: label.width, height: label.height }
}

function elkEdges(edges: Array<{ from: string; to: string; label?: string }>, prefix: string): ElkExtendedEdge[] {
...
function layoutEdges(
  ...
  prefix: string,
  offsetFor: (edge: { from: string; to: string }) => { x: number; y: number } = () => ({ x: 0, y: 0 }),
): LayoutEdge[] {
  return input.map((edge, index) => {
    ...
    return {
      ...
      labelBox: box ? { ...box, x: box.x + offset.x, y: box.y + offset.y } : undefined,
    }
  })
}
...
export async function layoutScreenFlow(spec: ScreenFlowSpec): Promise<Layout> {
  ...
  const nodes: LayoutNode[] = spec.screens.map((screen) => {
    ...
    if (!placed || placed.x === undefined || placed.y === undefined) throw new Error(`ELK node ${screen.id} is missing coordinates`)
    return {
      id: screen.id,
