...
function loadSample(fileName: string): DiagramSpec {
  const path = fileURLToPath(
    new URL(`../../samples/${fileName}`, import.meta.url)
  )
...
test("explicit kind wins and unknown explicit kind is generic", () => {
  const spec1: DiagramSpec = {
    type: "architecture",
    title: "A",
    nodes: [{ id: "n", icon: "api", kind: "external" }]
  }
  expect(decorateLayout(spec1, stubLayout(spec1)).nodes[0].kindKey).toBe(
    "external"
  )
  ...
  const spec2: DiagramSpec = {
    type: "architecture",
    title: "A",
    nodes: [{ id: "n", kind: "warehouse" }]
  }
  expect(decorateLayout(spec2, stubLayout(spec2)).nodes[0].kindKey).toBe(
    "generic"
  )
})
...
test("web-architecture.spec.json: 全ノードの kindKey スナップショット", () => {
  ...
  expect(
    result.nodes.map((n) => ({ id: n.id, kindKey: n.kindKey }))
