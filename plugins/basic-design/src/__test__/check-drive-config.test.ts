...
const BUNDLED_CLI = fileURLToPath(
  new URL("../../scripts/check-drive-config.mjs", import.meta.url)
)
...
test("BOM/CRLF/quoted id/comment を現行どおり読む", async () => {
  ...
  expect(readDriveConfig(root)).toEqual({
    configured: true,
    driveFolderId: "1AbC"
  })
})
...
test.each([
  ...
  expect(readDriveConfig(await project(content))).toEqual({
    configured: false,
    driveFolderId: null
  })
})
...
test("CLI 引数省略時は cwd を使い JSON 1行で exit 0", async () => {
  ...
  const { stdout, stderr } = await execFileAsync("node", [BUNDLED_CLI], {
    cwd: root
  })
  expect(stderr).toBe("")
