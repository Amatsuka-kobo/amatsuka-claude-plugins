...
// biome-ignore lint/suspicious/noExplicitAny: 不正な値へ意図的に書き換えるバリデーションテスト用フィクスチャ
const validSpec = (): any => ({
  type: "er",
  title: "受注管理 ER図",
  entities: [
...
test("title が無い・空はエラー", () => {
  ...
  expect(
    validateSpec({ ...validSpec(), title: "" }).some((e) => e.includes("title"))
  ).toBe(true)
})
...
test("エンティティ名の重複はエラー(重複した名前を含むメッセージ)", () => {
  ...
  expect(
    validateSpec(spec).some((e) => e.includes("users") && e.includes("重複"))
  ).toBe(true)
})
...
test("screen-flow: screens が空はエラー", () => {
  expect(
    validateSpec({ ...validScreenFlow(), screens: [] }).some((e) =>
      e.includes("screens")
    )
  ).toBe(true)
})
...
test("screen-flow: id 重複はエラー", () => {
  ...
  expect(
    validateSpec(spec).some((e) => e.includes("login") && e.includes("重複"))
  ).toBe(true)
})
...
test("screen-flow: transitions 省略可・非配列はエラー", () => {
  ...
  expect(validateSpec(spec)).toEqual([])
  expect(
    validateSpec({ ...validScreenFlow(), transitions: {} }).some((e) =>
...
test("architecture: nodes が空はエラー", () => {
  expect(
    validateSpec({ ...validArchitecture(), nodes: [] }).some((e) =>
      e.includes("nodes")
    )
  ).toBe(true)
})
...
test("architecture: ノードが複数ゾーンに属するとエラー", () => {
  ...
  expect(
    validateSpec(spec).some((e) => e.includes("db") && e.includes("複数"))
  ).toBe(true)
})
...
test("architecture: zone id とノード id の衝突はエラー", () => {
  ...
  const errors = validateSpec(spec)
  expect(errors.some((e) => e.includes('"db"') && e.includes("重複"))).toBe(
    true
  )
})
...
test("sequence: actors が空はエラー", () => {
  expect(
    validateSpec({ ...validSequence(), actors: [] }).some((e) =>
      e.includes("actors")
    )
  ).toBe(true)
})
...
test("sequence: 自己メッセージはエラー(未対応の明示)", () => {
  ...
  expect(validateSpec(spec).some((e) => e.includes("自己メッセージ"))).toBe(
    true
  )
})
...
test("sequence: 不正な style はエラー", () => {
  ...
  expect(
    validateSpec(spec).some((e) => e.includes("style") && e.includes("dashed"))
  ).toBe(true)
})
