...
import { readStdin, emit, pass, hasSkillInvocation, hasSkillFileRead, lastAssistantModel, subagentTranscriptPath } from "./lib.mjs";
...
const skillMdPath = (skill) => path.join(PLUGIN_ROOT, "skills", skill.split(":")[1], "SKILL.md");
...
try {
  ...
  if (agentId) {
    transcript = subagentTranscriptPath(input.transcript_path, input.session_id, agentId);
  ...
  const marker = path.join(dir, `${input.session_id}-${agentId ?? "main"}-${skill.replace(/[^a-zA-Z0-9-]/g, "_")}`);
