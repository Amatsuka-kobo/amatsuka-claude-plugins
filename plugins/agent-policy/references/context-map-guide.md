...
モデルの割り当ては各 SKILL の担当表が与える(with-codex / claude-only)。本 guide の規範文において、エージェントを指す場合はすべて役割名で書く
...
| 対象                   | 読む深さ                                                                                                                 | 根拠                                                                       |
| ---------------------- | ------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------- |
| **オーケストレーター** | 圧縮された map を**読む**(= 生探索の代替、設計目標)。チェックポイントごとの主担当・参照セクションは次節の表に従う        | map の主要消費者。読むこと自体がコスト削減                                 |
