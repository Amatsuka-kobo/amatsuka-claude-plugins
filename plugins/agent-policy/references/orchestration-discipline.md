...
オーケストレーターが担うのは dispatch・要件確定・採否判断・承認・分析である。担当表の全帯はサブエージェントの役割である。オーケストレーターはそのどれも自ら実行しない。
...
- サブエージェントは、アドバイザーとして相談する相手を、役割マーカーの対応表にある「設計・計画・実装のアドバイザー」の帯の定義にする。対応表に無いときは `Fable` にする。`Fable` が起動できないときは相談せず、差し戻しで解決する。
