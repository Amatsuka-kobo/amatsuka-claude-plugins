...
2. 担当が GPT 帯で GPT が利用可能なら(基本動作): 該当する役割 Agent 定義ファイルの本文(frontmatter を除く)を、担当 GPT エージェントへの依頼文に役割定義として同梱して dispatch する。dispatch 時の `model` 上書きは使わない。依頼文に「この tools のみ使用」と明記する。
