...
あなたはオーケストレーターまたはそのサブエージェントである。以下はモデル別の役割分担と進め方の規律である。この方針は Codex 系 GPT モデルをローカルプロキシ経由で使えること、または codex プラグインが使用できることを前提とする。GPT/Codex が使えない環境では、代わりに `agent-policy:claude-only` に従うこと。
