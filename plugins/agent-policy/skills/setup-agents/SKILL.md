...
選んだ方針の役割が、プロジェクトの既存定義でどこまで埋まっているかを取得する。
...
1 つでも `coveredBy` が空でないときは、「役割 / 担当表のモデル / 既存定義」の表を提示したうえで、次を聞く。
...
3. 「作る」を選ばれたら、使うモデルを決める。`models` が 1 件ならそのモデルを使い、質問しない。2 件以上なら、どのモデルを使うかを聞く。
4. 定義名を尋ねる。表示だけで済ませず、必ず質問する。既定は `<model-id>-<default-name>` である。`<default-name>` は被覆の応答が返した `defaultName` であり、ステップ 4 の `--list-models` が返す `defaultName` とは別物である。まとめた場合は、選んだ役割のうち被覆の応答の並びで最初のものを使う。選択肢は「既定名 `<name>` を使う」と「別の名前を指定する」の 2 つとし、後者が選ばれたら自由入力で受ける。
...
6. 生成する。
   ...
   ```bash
   node "${CLAUDE_PLUGIN_ROOT}/scripts/setup-agents.mjs" --write --policy <policy-id> --model-id <model-id> --lang <lang> --name <name> --roles <role-id,...> [--mcp-servers <server,...>] [--mcp-deny <tool,...>] --dir "$PWD"
   ```

7. 被覆を取り直し、残った対象について 1 へ戻る。

各書き込みの `results` を保存し、ステップ 7 の報告に使う。
