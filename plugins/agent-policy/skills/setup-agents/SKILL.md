...
4. 定義名を尋ねる。表示だけで済ませず、必ず質問する。既定は `<model-id>-<default-name>` である。まとめた場合の `default-name` は、選んだ役割のうち被覆の応答の並びで最初のものを使う。選択肢は「既定名 `<name>` を使う」と「別の名前を指定する」の 2 つとし、後者が選ばれたら自由入力で受ける。
