...
- 意訳では、例えば日本語では次のように言い換える。
  - Version を単に「版」とせず、「確定版」「新しいバージョン」のように文脈で使い分ける。
  - テスト結果の Green / Red を「緑」「赤」とせず、「パス」「失敗」と書く。
  - Frozen Document を「凍結文書」とせず、「確定版の文書」と書く。
  - Ledger を単に「台帳」とせず、網羅的に集めたものは「一覧」、状態の更新を伴うものは「管理表」、時系列の記録は「ログ」と書く。
  - Step を「段」とせず、そのまま「Step」または「ステップ」と書く。
