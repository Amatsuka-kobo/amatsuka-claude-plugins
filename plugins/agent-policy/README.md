...
11. **オーケストレーターが「文書作成」へ委譲するときの依頼文の形を定めました。** 依頼文には、事実・決定事項・制約・受け入れ基準・章立て・用語・参照先を、箇条書き・表・キーワードで渡します。対象ファイルの文体で書いた段落や、そのまま貼り付けられる完成文は載せません。文章を考えることが文書作成の役割だからです。文書作成の役割には、完成文を渡された場合も下書きとして自分の言葉で書き直す規律を加えました。生成する定義の共通規律にも、再委譲の依頼文へ完成文を載せない項目を加えています。生成済みの定義は自動では変わりません。文言を追随させるには `agent-policy:setup-agents` を再実行してください。
