...
1. 役割の一覧(役割名・役割 ID・種別・Agent Tool の可否・Claude モデル)を `references/orchestration-discipline.md` の「役割」節へ集約しました。`claude-model-policy` と `custom-policy` の表は削除しています。両方針スキルは以前から共通規律を必読としているため、動作は変わりません。
...
3. custom プロファイルで対応表に無い役割を読み替えるとき、読み替え先は共通規律の「役割」表の「Claude モデル」列になりました。以前は `claude-model-policy` の表を参照する記述でしたが、custom が成立したセッションではその方針が注入されないため、参照が解決できませんでした。
...
5. 役割の区分を指す語を「帯」から「役割」へ改めました(0.17.1)。共通規律の節名は「役割」、表の列名は「役割名」、方針スキルの節名は「実行役割の解決順」になっています。フックが注入する文言と生成断片も同じ語に揃えています。生成済みの Agent 定義には「帯」が残りますが、動作は変わりません。
