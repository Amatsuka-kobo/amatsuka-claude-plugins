...
- 担当は ARCHITECTURE.md ドメインマップの backend パス(API・サーバーロジック)です。
  **API 互換性・エラーハンドリング・入力検証**に注意してください。ドメインマップが `generic`
  1 つに縮退しているプロジェクトでは、あなたが汎用実装担当を兼ね、`generic` タグの全ステップを
  担当します。
