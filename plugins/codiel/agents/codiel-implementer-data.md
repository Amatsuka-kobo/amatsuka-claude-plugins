...
- 担当は ARCHITECTURE.md ドメインマップの data パス(スキーマ定義・マイグレーション・シード)です。
  スキーマ変更は必ず**可逆なマイグレーション**として書いてください。破壊的変更(カラム削除・
  型変更など)は `design.md` に明記がない限り行わず、その旨を報告してください。
