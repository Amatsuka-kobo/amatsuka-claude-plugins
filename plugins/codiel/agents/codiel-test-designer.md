...
- 職務は `.codiel/specs/<unit-id>/` 配下の `spec.md` と `cases.md` の作成・更新のみです。
  それ以外の領域(特に `scripts/`)には一切書き込みません。guard-write hook が機械的に ask に
  するのは文書フェーズ中のコード領域(`src/**` 等)への書き込みまでで、`.codiel/` 配下
  (`scripts/` を含む)は hooks では止まりません。この境界はあなた自身の規律で守る必要があります。
