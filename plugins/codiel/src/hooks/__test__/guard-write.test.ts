...
interface HookOutput { permissionDecision: string; permissionDecisionReason: string }

function hook(cwd: string, toolName: string, filePath: string): HookOutput | null {
  const input = JSON.stringify({ cwd, tool_name: toolName, tool_input: { file_path: filePath } });
  const out = runTs(HOOK, [], { input });
  if (out === "") return null;
  return (JSON.parse(out) as { hookSpecificOutput: HookOutput }).hookSpecificOutput;
}
function setupRun(phasesToPass: string[] = []): string {
  ...
  for (const ph of phasesToPass) {
    runTs(CLI, ["pass-gate", ph, "--issue", "1", "--evaluation-id", "e", "--verdict", "PROCEED"], { cwd: root });
...
test("文書フェーズ(init)中の src への書き込みは ask、.codiel 配下は素通し(無出力)", () => {
  const root = setupRun();
  expect(hook(root, "Write", path.join(root, "src/app.ts"))!.permissionDecision).toBe("ask");
  expect(hook(root, "Write", path.join(root, ".codiel/runs/issue-1/try-1/issue.md"))).toBe(null);
});
...
test("implement フェーズ中: src は素通し、specs の cases.md は ask", () => {
  ...
  expect(hook(root, "Edit", path.join(root, ".codiel/specs/screen-login/cases.md"))!.permissionDecision).toBe("ask");
  expect(hook(root, "Edit", path.join(root, ".codiel/specs/screen-login/spec.md"))!.permissionDecision).toBe("ask");
  expect(hook(root, "Write", path.join(root, ".codiel/specs/screen-login/scripts/login.spec.ts"))).toBe(null);
});
...
test("discuss フェーズ中: .codiel 配下(agenda.md/discussion.md)は素通し、src への書き込みは ask", () => {
  ...
  cli(["pass-gate", "init", "--issue", "1", "--evaluation-id", "e", "--verdict", "PROCEED"]);
  cli(["start-phase", "discuss", "--issue", "1"]);
  expect(hook(root, "Write", path.join(root, ".codiel/runs/issue-1/try-1/agenda.md"))).toBe(null);
  expect(hook(root, "Write", path.join(root, ".codiel/runs/issue-1/try-1/discussion.md"))).toBe(null);
