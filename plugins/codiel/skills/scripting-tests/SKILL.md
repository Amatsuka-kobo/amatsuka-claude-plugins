...
8. NG が出たケースは、cases.md の期待結果とスクリプトの記述が一致していることを再確認した
   ...
   **レポートには NG ケースごとに「ケース ID / 再現手順 / 期待結果(cases.md の記載)/
   実際の結果(実行出力の抜粋)」の 4 項目を必ず書く**。これは TDD 修正ループ (B) で
   implementer に渡される入力契約であり、欠けると修正がディスパッチできない
   (レポート全体の書式は `running-regression-tests` スキルが定める)。
   スクリプトの欠陥か NG か判断がつかないケースは broken に分類し、レポートに
   「判定不能(ASK)」として理由を明記する。
