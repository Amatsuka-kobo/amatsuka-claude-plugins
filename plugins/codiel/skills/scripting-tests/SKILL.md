...
- 使用フレームワーク: 次の順で解決する。解決できないことを理由に着手を止めない。
  1. コンテキストに E2E フレームワークの宣言があればそれに従う。
  2. 無ければ `package.json` の `devDependencies` から推定する(`@playwright/test`・`cypress` 等)。
     推定で決めたときは、フレームワーク名と推定の根拠をレポートに書く。
  3. 推定もできなければ AskUserQuestion で使うフレームワークを問う。
- 解決したフレームワークを途中で自己判断で変えない。
