...
- 「ライブラリの一般的な API を型を確認せず書いて存在しないメソッドを呼んだ」ことそのものは
  一般的な AI の失敗傾向だが、「**このプロジェクトのどの ORM/バージョンで**それが起きたか」は
  プロジェクト固有情報として記録に値する(GOTCHA-001 参照)。判断が割れる場合は「次回同じ
  プロジェクトファイルを触るエージェントへの手がかりになるか」で判定する。
...
7. `docs/GOTCHAS.md` を `git add` し、`codiel(gotchas): <一行要約>` の形式でコミットする
   (SKILL.md 自体の変更コミットとは別物であり、run 中の実運用コミットに使う書式)。
