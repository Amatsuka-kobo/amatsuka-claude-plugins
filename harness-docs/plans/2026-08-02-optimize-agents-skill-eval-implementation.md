...
中間コミット 2 本のメッセージに `0.11.1-dev` / `0.11.2-dev` とあるが、これは作業途中でサブエージェントが繰り上げた値である。最終的な `plugin.json` は **`0.11.0-dev`** に揃えた。この一連の変更全体で 1 つのマイナーバージョン繰り上げとして扱う。

| コミット | メッセージの表記 | 実際 |
| --- | --- | --- |
| `964f4ce` | 0.11.1-dev | 作業途中 |
| `45c8ccb` | 0.11.2-dev | 作業途中 |
| `091f640` | 0.11.0-dev | **最終** |
