...
### #4 検証 2 の期待値は母集団の代表性に依存する(ステップ 7 で発見)

- **事実**: ステップ 7 の完了時点で `.raphael/commands.jsonl` は一意 132 件まで育っており、下限 50 件を超えて広さ検査が働きます。この母集団に対して現行 56 抗体の一致率を実測したところ、既定閾値 10% を超えたのは `ab-2026-0827-002`(13.64%、pattern `git.*(diff|add|checkout|show).*\.(ts|js|md)`)と `ab-2026-0907-001`(11.36%、pattern `pnpm\s+(run\s+)?(lint|test)`)の 2 件でした。**§6 の検証 2 が期待する `ab-2026-0803-002`(pattern `&&.*[|;]`)は 4.55% にとどまり、`noisy` になりません。**
- **原因**: 母集団の性質が設計書 §1.1 の事実 F7 と異なります。F7 は 137 セッション・一意 1,514 件の一般作業を母集団としており、そこでは `&&.*[|;]` が 23.96% で最広でした。一方、現在の `commands.jsonl` は本改修の実装セッション 1 回分だけで構成されており、`git diff -- <path>` と `pnpm run lint` / `pnpm run test` に強く偏っています。そのため git 系と pnpm 系の pattern が上位に来て、複合コマンドの pattern が沈みます。
