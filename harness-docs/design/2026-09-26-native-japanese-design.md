...
agent-policy の「文書の執筆」節から移し、節 4 の構文の表の後に「語 / 避ける訳 / 使う訳」の表として置く。実装当初は独立した節 6 だったが、2026-09-26 に節 4 へ統合した(節は 5 つになる)。
...
agent-policy は、オーケストレーター向けの規律と各 Agent 定義の共通断片の両方に、文書の書き方を持っている。本プラグインが SessionStart と SubagentStart の両方で同じ種類の規律を届けるため、残すと二重になる。訳語の 5 項は `discipline.md` の節 4 へ移る。
