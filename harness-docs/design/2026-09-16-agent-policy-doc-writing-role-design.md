...
### 2.6 文書作成の依頼文で完成文を渡さない(追加要件 G。ユーザー承認済み 2026-09-17)

1. **事象**: オーケストレーターが「文書作成」の役割へ委譲するとき、書くべき文面を依頼文に全文指定したため、委譲先に文章を考える余地が無くなった。`docs/chat/2026/0916/phyllis998/1620-agent-policy-role-id-design.md:1586` で、オーケストレーター自身が「書く文面を私が全文指定している」と記録している。
2. **原因**: 次の 4 層が重なった。(1) 既存条項は依頼文に渡すものを列挙するだけで、渡してはいけないものを定めていなかった。(2) 「内容の決定」と「執筆」の境界に判定手続きが無く、決定を細かくすると文面に到達した。(3) 全文指定には出力を確実にできるという積極的な誘因があり、禁止が無ければリスク回避の合理的な帰結として選ばれ続ける。(4) 受け手側にも、文面まで渡された逆流を検出する条項が無かった。既存の「内容の決定は依頼元が持つ」は、写経を許す読み方を与えていた。
3. **対処**: 依頼文へ渡してよいものと渡してはいけないものを、内容の粒度ではなく形式で定める。箇条書き・表・キーワードで事実や決定を渡し、対象ファイルの文体で書かれた段落や完成文は渡さない。この規律をオーケストレーター側と文書作成側の両方に置く。
...
### 4.17 完成文を渡さない規律の判定方法と置き場所

**形式で判定する。** 依頼文に箇条書き・表・キーワードを使うことは許可する。一方、対象ファイルの文体で書かれた段落と、そのまま貼り付けられる完成文は載せない。内容の決定と執筆の境界を、情報の内容ではなく依頼文の形式で判定できるようにする。

**オーケストレーター側と文書作成側の両方に置く。** 片側だけでは、全文指定で出力を確実にしたい誘因が残る。また、箇条書きの粒度を上げて実質的な文面を渡す潜脱を受け止められない。受け手が完成文を自分の言葉で書き直すと定めれば、渡してもそのまま使われないため、全文指定の誘因自体が消える。

### 4.18 §オーケストレーターが自ら担う作業 の小見出し分割

**決定: `references/orchestration-discipline.md` の §オーケストレーターが自ら担う作業を、`### 分析` と `### 文書作成を委譲するとき` の 2 つに分ける。** 前者にはオーケストレーター自身が担う分析を、後者には文書作成の委譲だけに適用する規律を置く。

**分ける理由。** この節には、オーケストレーター自身の分析と文書作成の委譲という 2 つの主題が平らな箇条書きで並んでいた。完成文を渡さない条項を加えた後も、見出しから適用対象を読めないため、文書作成以外の委譲にも掛かると誤解されうることが、ユーザーの指摘で分かった。小見出しで対象を分け、各条項にも「文書作成を委譲する依頼文」または「文書作成の委譲先」と明記する。条項だけを引用しても適用範囲が保たれる。

**§4.12 との区別。** ja 断片で `###` 見出しを使わない判断は、合成時に他の役割の箇条書きへ紛れ込むことを避けるための `assets/roles/ja` に限る規則である。この規律文書には適用しない。`references/orchestration-discipline.md` は他の節でも `###` を使っており、今回の小見出しはその構成に従う。
...
1. `references/orchestration-discipline.md` の §オーケストレーターが自ら担う作業にある `### 文書作成を委譲するとき` で、既存の「文書作成を委譲する依頼文には、確定した内容・対象ファイルのパス・参照すべき既存文書を渡す。…」の直後へ次の 3 条項を挿入する。

   ```markdown
   - 文書作成を委譲する依頼文には、事実・決定事項・制約・受け入れ基準・章立て・用語・参照先のパスだけを、箇条書き・表・キーワードの形で書く。
   - 文書作成を委譲する依頼文には、対象ファイルの文体で書かれた段落や、そのまま貼り付けられる完成文を載せない。
   - 文章を考えること自体が、文書作成の委譲先の役割である。
   ```

2. `assets/roles/ja/_common.md` の `## Agent tool の制約` で、再委譲の依頼文に書く項目の列挙へ、`対象ファイルのパス` の直後に次を挿入する。

   ```markdown
   - 対象ファイルにそのまま使える完成文を依頼文に載せないこと
   ```

3. `assets/roles/en/_common.md` の `## Agent tool limits` で、再委譲の依頼文に書く項目の列挙へ、`the paths of the target files` の直後に次を挿入する。

   ```markdown
   - that the request must not include finished prose ready to copy into the target files
   ```

   英語側は既存の列挙に合わせ、各項目を `that` 節で書く。

4. `assets/roles/ja/doc-writing.md` の `## 制約` で、既存の「文書の執筆を他の役割へ再委譲しない。…」の直後に次を挿入する。

   ```markdown
   - 依頼元から対象ファイルの文体で書かれた完成文を渡されたときは、下書きとして自分の言葉で書き直し、そのまま書き写さない。
   ```

5. `assets/roles/en/doc-writing.md` の `## Constraints` で、既存の `Do not re-delegate the writing to another role. This role does the writing.` の直後に次を挿入する。
...
| コピペテストを完成文の判定手続きにする | 依頼文の該当部分をそのまま対象ファイルへ貼り付けて文書として成立するなら渡しすぎとする案である。実際に観測された事象は対象ファイルの文体で書かれた段落であり、形式の規律だけで捕捉できる。これ以上の判定手続きは過剰であるとユーザーが判断した(§4.17) |
| 規律をオーケストレーター側だけ、または文書作成側だけに置く | 全文指定で出力を確実にしたい誘因と、箇条書きで実質的な文面を渡す潜脱のいずれかが残る。受け手が書き直す規律を持たなければ、誘因そのものも消えない(§4.17) |
...
5. `RECOMMENDED["doc-writing"]` に入れた `gemini-flash` と `gpt-terra` が日本語の文書執筆でどの程度の品質を出すかは未検証である。運用で不足が分かれば推奨を見直す。
   ...
   **2026-09-17 決着。** `RECOMMENDED["doc-writing"]` から `gemini-flash` を外さず、README に注記する方を採った(ユーザー判断)。agent-policy はエイリアスがどの経路で配られているかを知り得ないため、推奨から外すと Antigravity 以外の経路で Gemini を配っている構成まで巻き添えにする。注記は `plugins/agent-policy/README.md` の推奨モデル ID の表の直後に置き、Antigravity 経由ではサブエージェントとして起動できないこと、メインセッションでは動作すること、Antigravity のログインを第三者クライアントから使うことが Google の利用規約で禁じられていること、回避を試みないこと、代替として API キーの経路があることを書いた。追加調査の結果として、定額のサブスクリプションで第三者クライアントから Gemini を呼べる Google の商品は確認した範囲では存在せず、Google AI Pro の「AI Studio の上限拡大」も AI Studio の Web UI の中だけに適用されることが分かっている。`gpt-terra` の日本語品質については引き続き観測を続ける。
...
- `references/orchestration-discipline.md` の §オーケストレーターが自ら担う作業に、文書作成の依頼文で許可する形式、対象ファイルの文体で書かれた段落と完成文を載せないこと、文章を考える役割の所在を定めた 3 条項がある。
- `assets/roles/{ja,en}/_common.md` の再委譲の依頼文に書く項目の列挙に、対象ファイルへそのまま使える完成文を載せない旨がある。英語側は既存の `that` 節の形に従っている。
- `assets/roles/{ja,en}/doc-writing.md` の制約に、依頼元から渡された完成文を下書きとして自分の言葉で書き直し、そのまま書き写さない旨がある。
